...
// Получаем диалоги //
$input = curl("https://api.vk.com/method/messages.getDialogs?count=10&access_token=" . $token);

// Профиль онлайн, когда скрипт работает //
curl("https://api.vk.com/method/account.setOnline?access_token=" . $token);
...
<?
    ...
    if ($input['response'][$i]['out'] == '0') {
        ...
        if ($vkmessage[0] == '/'){
            ...
            curl("https://api.vk.com/method/messages.markAsRead?access_token=" . $token . "&peer_id=" . $uid);
            sleep(3);
            curl("https://api.vk.com/method/messages.setActivity?access_token=" . $token . "&user_id=" . $uid . "&type=typing");
            sleep(4);
        ...
        elseif ($row['vkid'] == $uid) {
            ...
            // Устанавливаем имя //
            curl('http://bot.mew.su/sp.php?session=' . $row2['chatid'] . '&text=' . urlencode('!botsetname ' . $row2['firstname']));

            // Устанавливаем пол //
            if ($row2['sex'] == '2'){
                curl('http://bot.mew.su/sp.php?session=' . $row2['chatid'] . '&text=' . urlencode('я мальчик'));
            } elseif ($row2['sex'] == '1') {
                curl('http://bot.mew.su/sp.php?session=' . $row2['chatid'] . '&text=' . urlencode('я девочка'));
            }
        }
    }
